...
class ReplayBuffer:
    ...
    def _reset(self):
        ...
        # self._statistics = np.empty((self._memory_size, 1), dtype=np.float32)
    ...
    def _append(self, state, action, reward, next_state, done, sim_state=None, episode_done=None):
...
class TemporalPrioritizedReplayBuffer(ReplayBuffer):
    ...
    def _reset(self):
        ...
        self._statistics = np.empty((self._memory_size, 1), dtype=np.float64)
        self._cur_done_cnt = 0

    def _append(self, state, action, reward, next_state, done, step, statistic, sim_state=None, episode_done=None):
        super()._append(state, action, reward, next_state, done, sim_state, episode_done=episode_done)
        # We can compute mod on negative number
        self._p = (self._p - 1) % self._memory_size 
        self._steps[self._p, ...] = step
        self._statistics[self._p, ...] = statistic
...
class BackTimeBuffer(TemporalPrioritizedReplayBuffer):
    ...
    def append(self, state, action, reward, next_state, done, step=None, episode_done=None, statistic=None, sim_state=None):
        ...
        self._append(state, action, reward, next_state, done, 0, statistic, sim_state)
